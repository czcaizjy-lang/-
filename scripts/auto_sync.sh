#!/bin/bash
# 自动同步脚本：由 launchd WatchPaths 触发
# Excel 保存 → 消抖 → 同步(通过 osascript 获取桌面权限) → 构建 → 推送

set -e

PROJECT_DIR="/Users/xiaocao/CC/每日业绩自动统计"
...
log "--- 同步数据 ---"
osascript <<EOF
do shell script "cd '$PROJECT_DIR' && /usr/bin/python3 scripts/sync_dashboard.py >> '$LOG_FILE' 2>&1"
EOF
if [ $? -ne 0 ]; then
    log "✗ 数据同步失败，终止"
    exit 1
...
# Step 2: 构建独立页面
log "--- 构建页面 ---"
python3 scripts/build_standalone.py >> "$LOG_FILE" 2>&1 || {
    log "✗ 页面构建失败，终止"
    exit 1
}
...
# Step 3: Git 推送
log "--- 推送部署 ---"
git add data/dashboard_data.json docs/index.html 2>/dev/null

if git diff --cached --quiet 2>/dev/null; then
    log "无数据变更，跳过 git push"
else
    git commit -m "📊 数据自动更新 $(date '+%Y-%m-%d %H:%M')" >> "$LOG_FILE" 2>&1
